...
def graph_eval(data_dir,input_graph_def, graph, input_node, output_node, batchsize):
    ...
    labels = tf.compat.v1.placeholder(tf.uint8,shape = [None,])
    ...
    tf_acc, tf_acc_update = tf.compat.v1.metrics.accuracy(labels, predicted_logit, name='accuracy')
    ...
    with tf.compat.v1.Session() as sess:
      ...
      print('Graph accuracy with test dataset: {0:.2f}'.format(score*100),'%')
...
def main():
    ...
    ap.add_argument('-d', '--data_dir',    type=str, default='data',            help='path to folder containing testData.npz')
    ...
    graph_eval(args.data_dir,input_graph_def, args.graph, args.input_node, args.output_node, args.batchsize)
