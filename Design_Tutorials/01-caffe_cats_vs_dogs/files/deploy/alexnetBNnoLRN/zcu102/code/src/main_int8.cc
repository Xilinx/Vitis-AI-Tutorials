...
const string wordsPath     = "./code/src/";
...
void run_CNN(vart::Runner* runner) {
  ...
  //float mean[3] = {106.40268, 116.04212, 124.45967};
  float mean[3] = {104, 114, 123};
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      for (int h = 0; h < inHeight; h++) {
        for (int w = 0; w < inWidth; w++) {
          for (int c = 0; c < 3; c++) {
	    float tmp = ((float)image2.at<Vec3b>(h, w)[c]) - mean[c];
	  imageInputs[i*inSize+h*inWidth*3+w*3 +  c] = (int8_t) ( tmp * input_scale); //in BGR mode
	  //imageInputs[i*inSize+h*inWidth*3+w*3 +2-c] = (int8_t) ( tmp * input_scale); //in RGB mode
          }
        }
      }
...
int main(int argc, char* argv[])
{
  // Check args
  if (argc != 2) {
    cout << "Usage: run_cnn xmodel_pathName" << endl;
    return -1;
  }
