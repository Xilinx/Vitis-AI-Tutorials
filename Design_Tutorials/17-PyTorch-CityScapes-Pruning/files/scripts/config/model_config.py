...
class Options():
    def __init__(self):
        ...
        parser.add_argument('--dataset', type=str, default='citys_reduced',help='dataset name (default: cityscapes)')
        parser.add_argument('--eval_pruned', type=bool, default=False, help='run a validation process on the pruned model')
        parser.add_argument('--prune', type=bool, default=False, help='run pruning process on the model')
        parser.add_argument('--prune_model_py', type=str, default=None, help='prune model py file name without .py extension (default: None)') #'pruned_model_defs.pruned_fpn_resnet_0_',help='prune model py file name without .py extension (default: None)')
        parser.add_argument('--pruned_weights', type=str, default=None, help='prune model sparse weights (default: None)')
        parser.add_argument('--num-classes', type=int, default=6, help='the classes numbers (default: 19 for cityscapes)')
        parser.add_argument('--data-folder', type=str, default=None,help='training dataset folder (default: ./data)')
        parser.add_argument('--ignore_label', type=int, default=-1, help='the ignore label (default: 255 for cityscapes)')
        parser.add_argument('--prune_iterations', type=int, default=5, help='the number of iterations for which to run pruning')
        parser.add_argument('--prune_ratio', type=float, default=0.1, help='the pruning ratio - each iteration will increase by this amount')
