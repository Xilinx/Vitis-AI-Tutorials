...
void ref_SoftMax(signed char  *inp_data, float *out_data, float scale_fact, unsigned char size)
{
...
//		float *out_softmax,
		unsigned char *out_max, unsigned char *out_index, float scale_fact, unsigned short int height, unsigned short int width)

{
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      ...
      ref_SoftMax(ch_vect, softmax, scale_fact, size);
