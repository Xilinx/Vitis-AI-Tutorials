...
//this tutorial applies 12 classes only, but the post processor PL accelerator was dsigned for 28 classes
int MAX_NUM_CLASSES=28;
...
int main(int argc, char *argv[])
{
  ...
  cout << "\n" << endl;
  auto hwPre  = new HW_PRE();
  auto hwPost = new HW_POST();
  hwPre->init("hls_dpupreproc_m_axi");
  hwPost->init("hls_dpupostproc_m_axi");
  hwPre->load();
  hwPost->load();
  cout << endl;
  ...
  */
  ...
  sprintf(tempbuf1, "%s", "./data_pre2post/post_int8_din.bin");
  ...
  sprintf(tempbuf2, "%s", "./data_pre2post/post_uint8_out_max.bin");
  Write_Bin_File_as_Uchar(tempbuf2, osize*num_of_images, max_out);
  sprintf(tempbuf2, "%s", "./data_pre2post/post_uint8_out_idx.bin");
  Write_Bin_File_as_Uchar(tempbuf2, osize*num_of_images, idx_out);
  sprintf(tempbuf2, "%s", "./data_pre2post/post_uint8_ref_idx.bin");
...
/////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //close Pre- and Post-processing  kernels
  hwPre->close();
  cout << "deleting hwPre       memory" << endl;
  //delete hwPre;
  cout << "deleting hwPost      memory" << endl;
  //delete hwPost;
