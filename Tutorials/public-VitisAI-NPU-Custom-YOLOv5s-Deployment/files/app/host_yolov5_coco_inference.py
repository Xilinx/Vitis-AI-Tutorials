...
# It currently supports processing ANY batch size (BS>=1)
# The script outputs the raw results from the YOLOv5 model to /tmp/yolov5_output.raw.

"""
Usage:
cd files/yolov5 
python3 ../app/host_yolov5_coco_inference.py --images_dir ../datasets/coco/images/val2017 --json ../app/host_res/coco_10images_detections.json   --NbImages 10
python3 ../app/host_yolov5_coco_inference.py --images_dir ../app/images                   --json ../app/host_res/zidane_bus_coco_detections.json --NbImages  2
"""
...
'''
# Example usage
example_category_name = 'person'  # Change this to any category name you're interested in
category_id = category_map.get(example_category_name)

if category_id is not None:
    print(f"The category ID for '{example_category_name}' is: {category_id}")
else:
    print(f"'{example_category_name}' category not found.")
'''
...
# ****************************************************************************************
# MAKE PREDICTIONS (aka DETECTIONS)
# ****************************************************************************************
...
def main_inf(inp_images_dir, inp_detections_file, NbImages, dump_output, batch_size):
    dump_input  = False
    ...
    # image_files.sort()
    selected_files = image_files[:NbImages]
    ...
    for batch_start in range(0, total_images, batch_size):
        batch_files = selected_files[batch_start:batch_start+batch_size]
        ...
        for image_file in batch_files:
            ...
            images_list.append(prepr_image)
            original_sizes.append(original_size)
            image_paths_list.append(image_path)

        # Convert batch to tensor
        batch_tensor = np.vstack(images_list)
        ...
        for i, pred in enumerate(results):
            ...
            image_id_str = os.path.basename(image_path).split('.')[0]
            image_id = int(image_id_str) if image_id_str.isdigit() else -1

            print(f"\nProcessing image {batch_start + i + 1}/{total_images} from batch {batch_start}: {image_path}")

            start_postproc = time.time()
            detections = detect_and_annotate(
                pred, image_path, original_size,
                conf_thres=CONF_THRESHOLD,
                iou_thres=IOU_THRESHOLD,
                dump_output=dump_output
            )
            end_postproc = time.time()
            detections_time = (start_postproc - end_postproc) * 1000   
    ...
    print(f"Total Inference Time on {total_images} images = {total_inference_time:.2f} ms")
    print(f"Total PostProc  Time on {total_images} images = {total_postproc_time:.2f} ms") 
    ...
    print(f"Detections saved to {inp_detections_file}")        

def main():
    # Set up the main argument parser
    parser = argparse.ArgumentParser(description="YOLOv5 Detection HOST Script") 
    ...
    # run main inference
    main_inf(args.images_dir, args.json, args.NbImages, args.dump_out, args.batch_size)
