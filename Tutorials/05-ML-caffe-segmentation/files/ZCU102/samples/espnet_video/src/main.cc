...
#define AVGPOOL_INPUT1 "AvgPool2d_1"	
#define AVGPOOL_INPUT2 "AvgPool2d_2"									
#define CONV_OUTPUT_NODE "ConvNd_85"
...
void runSegmentation(DPUTask *task, bool &is_running) {
    ...
    while (is_running) {
		...
		resize(img, img, Size(inWidth, inHeight), 0, 0, INTER_NEAREST);
		dpuSetInputImageWithScale(task, (char *)CONV_INPUT_NODE, img, mean, scale,0);
		dpuSetInputImageWithScale(task, (char *)AVGPOOL_INPUT1, img, mean, scale,0);							   
		dpuSetInputImageWithScale(task, (char *)AVGPOOL_INPUT2, img, mean, scale,0);
